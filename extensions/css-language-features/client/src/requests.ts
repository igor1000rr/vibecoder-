...
export function serveFileSystemRequests(client: BaseLanguageClient, runtime: Runtime) {
	client.onRequest(FsContentRequest.type, (param: { uri: string; encoding?: string }) => {
		...
		return workspace.fs.readFile(uri).then(buffer => {
			// Variance issue в новых @types/node на Node 22: workspace.fs.readFile()
			// возвращает Uint8Array<ArrayBufferLike>, а TextDecoder.decode() ожидает
			// ArrayBuffer или ArrayBufferView. Используем `as any` чтобы обойти
			// type-mismatch — runtime семантика идентична.
